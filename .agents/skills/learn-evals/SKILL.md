...
Start with the current workspace. If its root (or an ancestor of the working directory) contains the `fafo-evals` package and the course exercises, use that checkout; a fork or renamed directory is fine. When both a project copy and an installed copy of this skill are available, use the checkout's `.agents/skills/learn-evals/SKILL.md` so the lesson guide matches the exercises. Do not clone again or install this skill into the checkout. Preserve existing work.

If invoked from another project in course mode, look for an existing course checkout before helping choose a destination and cloning `https://github.com/pandemicsyn/fafo` into a new directory. A standalone installed skill contains teaching material, not the runnable app. Apply mode works in the learner's own app and does not need a course checkout.

When the learner is ready to run an exercise, read the checkout's `AGENTS.md` and `docs/usage.md`, check Node >=22.19, and install dependencies with `npm ci` if needed. Explain `npm run examples` before running it and follow the current lesson's pacing. Setup and initial lessons make no API calls. When writing vitest-evals code, suggest the official implementation skill with `npx skills add getsentry/vitest-evals` if absent; consult https://vitest-evals.sentry.dev/docs as a fallback. Do not make installing a second skill an onboarding step.
